...
export default function OrganizationSettingsCard() {
  ...
  const { userType, organizationSettings, setOrganizationSettings } = useUser();
  
  const [tempSettings, setTempSettings] = useState(organizationSettings);
  
  const handleSave = () => {
    setOrganizationSettings(tempSettings);
    console.log("Saving organization settings...", tempSettings);
    closeModal();
  };

  const handleCancel = () => {
    setTempSettings(organizationSettings);
    closeModal();
  };
        ...
        <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
          <div className="px-2 pr-14">
            <h4 className="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">
              Uredi podešavanja organizacije
            </h4>
            <p className="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
              Kontrolišite šta korisnici u vašoj organizaciji mogu da vide u sidebaru.
            </p>
          </div>
                ...
                <div className="space-y-4">
                  <div className="p-4 border border-gray-200 rounded-lg dark:border-gray-700">
                    <label className="flex items-center gap-3 cursor-pointer">
                      <input
                        type="checkbox"
                        checked={tempSettings.usersCanSeeMojaFirma}
                        onChange={(e) => setTempSettings({
                          ...tempSettings,
                          usersCanSeeMojaFirma: e.target.checked
                        })}
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          <Badge color="success" variant="light">Moja Firma</Badge>
                          <span className="font-medium text-gray-800 dark:text-white/90">Moja Firma sekcija</span>
                        </div>
                        <p className="text-sm text-gray-500 dark:text-gray-400">
                          Korisnici mogu da vide i pristupe sekciji "Moja Firma"
                        </p>
                      </div>
                    </label>
                  </div>
                  ...
                  <div className="p-4 border border-gray-200 rounded-lg dark:border-gray-700">
                    <label className="flex items-center gap-3 cursor-pointer">
                      <input
                        type="checkbox"
                        checked={tempSettings.usersCanSeeKomitenti}
                        onChange={(e) => setTempSettings({
                          ...tempSettings,
                          usersCanSeeKomitenti: e.target.checked
                        })}
                        className="w-4 h-4 text-brand-600 bg-gray-100 border-gray-300 rounded focus:ring-0 dark:bg-gray-700 dark:border-gray-600"
                      />
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          <Badge color="warning" variant="light">Komitenti</Badge>
                          <span className="font-medium text-gray-800 dark:text-white/90">Komitenti sekcija</span>
                        </div>
                        <p className="text-sm text-gray-500 dark:text-gray-400">
                          Korisnici mogu da vide i pristupe sekciji "Komitenti"
                        </p>
                      </div>
                    </label>
                  </div>
                  ...
                  <div className="p-4 border border-gray-200 rounded-lg dark:border-gray-700">
                    <label className="flex items-center gap-3 cursor-pointer">
                      <input
                        type="checkbox"
                        checked={tempSettings.usersCanSeeOstalo}
                        onChange={(e) => setTempSettings({
                          ...tempSettings,
                          usersCanSeeOstalo: e.target.checked
                        })}
                        className="w-4 h-4 text-brand-600 bg-gray-100 border-gray-300 rounded focus:ring-0 dark:bg-gray-700 dark:border-gray-600"
                      />
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          <Badge color="info" variant="light">Ostalo</Badge>
                          <span className="font-medium text-gray-800 dark:text-white/90">Ostalo sekcija</span>
                        </div>
                        <p className="text-sm text-gray-500 dark:text-gray-400">
                          Korisnici mogu da vide i pristupe sekciji "Ostalo"
                        </p>
                      </div>
                    </label>
                  </div>
