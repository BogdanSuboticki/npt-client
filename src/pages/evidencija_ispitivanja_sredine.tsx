...
interface TableRow {
  redniBroj: number;
  predmetIspitivanja: string;
  brojMernihMesta: string;
...
const initialRows: TableRow[] = Array(10).fill(null).map((_, idx) => ({
  redniBroj: idx + 1,
  predmetIspitivanja: '',
  brojMernihMesta: '',
  brojStrucnogNalaza: ['', '', '', ''],
...
const EvidencijaIspitivanjaSredine: React.FC = () => {
  ...
  const handleCellChange = (rowIdx: number, field: keyof TableRow, value: string) => {
    setRows(prev => {
      const updated = [...prev];
  ...
  const handleSubCellChange = (rowIdx: number, field: 'brojStrucnogNalaza' | 'datumIspitivanja' | 'datumSledecegIspitivanja' | 'napomena', subIdx: number, value: string) => {
    setRows(prev => {
      const updated = [...prev];
      const arr = [...updated[rowIdx][field]];
      arr[subIdx] = value;
      updated[rowIdx] = { ...updated[rowIdx], [field]: arr };
      return updated;
    });
  };
  ...
  const handleItemsPerPageChange = (newItemsPerPage: number) => {
    if (newItemsPerPage < itemsPerPage) {
      const hasDataInRemovedRows = rows.slice(newItemsPerPage).some(row =>
        row.predmetIspitivanja || row.brojMernihMesta || row.brojStrucnogNalaza.some(Boolean) || row.datumIspitivanja.some(Boolean) || row.datumSledecegIspitivanja.some(Boolean) || row.napomena.some(Boolean)
      );
    ...
    setRows(prev => {
      if (prev.length < newItemsPerPage) {
        const newRows = Array(newItemsPerPage - prev.length).fill(null).map((_, idx) => ({
          redniBroj: prev.length + idx + 1,
          predmetIspitivanja: '',
          brojMernihMesta: '',
          brojStrucnogNalaza: ['', '', '', ''],
          datumIspitivanja: ['', '', '', ''],
  ...
  const handlePrint = () => {
    ...
    subtitle.textContent = 'Evidencija o izvršenim ispitivanjima uslova radne sredine';
  ...
  const handleDownload = () => {
    ...
    const opt = {
      margin: 1,
      filename: 'evidencija_ispitivanja_sredine.pdf',
    ...
    <div className="py-8">
      <div className="flex-row md:flex-row md:justify-between md:items-center mb-6 gap-4">
        ...
        <h2 className='text-lg text-gray-400 dark:text-white mt-2'>
          Evidencija o izvršenim ispitivanjima uslova radne sredine
        </h2>
      </div>
      <div className="rounded-lg bg-white dark:bg-gray-800 shadow-[0_0_5px_rgba(0,0,0,0.1)]">
        ...
        <div ref={tableRef} className="overflow-x-auto">
          <table className="min-w-full border border-gray-200 dark:border-white/[0.1]">
            <thead>
              <tr>
                <th className="border border-gray-200 dark:border-white/[0.1] text-[13px] font-medium text-gray-700 dark:text-gray-400 px-4 py-3">Redni broj</th>
                <th className="border border-gray-200 dark:border-white/[0.1] text-[13px] font-medium text-gray-700 dark:text-gray-400 px-4 py-3">Predmet preventivnih i periodičnih ispitivanja</th>
                <th className="border border-gray-200 dark:border-white/[0.1] text-[13px] font-medium text-gray-700 dark:text-gray-400 px-4 py-3">Broj mernih mesta</th>
                <th className="border border-gray-200 dark:border-white/[0.1] text-[13px] font-medium text-gray-700 dark:text-gray-400 px-4 py-3">Broj stručnog nalaza/izveštaja</th>
                <th className="border border-gray-200 dark:border-white/[0.1] text-[13px] font-medium text-gray-700 dark:text-gray-400 px-4 py-3">Datum ispitivanja</th>
                <th className="border border-gray-200 dark:border-white/[0.1] text-[13px] font-medium text-gray-700 dark:text-gray-400 px-4 py-3">Datum sledećeg ispitivanja</th>
                <th className="border border-gray-200 dark:border-white/[0.1] text-[13px] font-medium text-gray-700 dark:text-gray-400 px-4 py-3">Napomena</th>
              </tr>
            </thead>
            <tbody>
              {rows.slice(0, itemsPerPage).map((row, idx) => (
                <React.Fragment key={idx}>
                  <tr>
                    ...
                    <td 
                      ...
                      onClick={() => handleCellClick(`predmetIspitivanja-${idx}`)}
                    >
                      <input 
                        type="text" 
                        className="w-full h-full outline-none bg-transparent text-[13px] text-gray-800 dark:text-gray-400 px-4 py-3" 
                        value={row.predmetIspitivanja} 
                        onChange={e => handleCellChange(idx, 'predmetIspitivanja', e.target.value)}
                        ref={(el) => { inputRefs.current[`predmetIspitivanja-${idx}`] = el; }}
                        data-input-key={`predmetIspitivanja-${idx}`}
                      />
                    </td>
                    <td 
                      rowSpan={4} 
                      className="border border-gray-200 dark:border-white/[0.1] p-0 cursor-text"
                      onClick={() => handleCellClick(`brojMernihMesta-${idx}`)}
                    >
                      <input 
                        type="text" 
                        className="w-full h-full outline-none bg-transparent text-[13px] text-gray-800 dark:text-gray-400 px-4 py-3" 
                        value={row.brojMernihMesta} 
                        onChange={e => handleCellChange(idx, 'brojMernihMesta', e.target.value)}
                        ref={(el) => { inputRefs.current[`brojMernihMesta-${idx}`] = el; }}
                        data-input-key={`brojMernihMesta-${idx}`}
                      />
                    </td>
                    <td 
                      className="border border-gray-200 dark:border-white/[0.1] p-0 cursor-text"
                      onClick={() => handleCellClick(`brojStrucnogNalaza-${idx}-0`)}
                    >
                      <input 
                        type="text" 
                        className="w-full h-full outline-none bg-transparent text-[13px] text-gray-800 dark:text-gray-400 px-4 py-3" 
                        value={row.brojStrucnogNalaza[0]} 
                        onChange={e => handleSubCellChange(idx, 'brojStrucnogNalaza', 0, e.target.value)}
                        ref={(el) => { inputRefs.current[`brojStrucnogNalaza-${idx}-0`] = el; }}
                        data-input-key={`brojStrucnogNalaza-${idx}-0`}
                      />
                    </td>
                    <td 
                      className="border border-gray-200 dark:border-white/[0.1] p-0 cursor-text"
                      onClick={() => handleCellClick(`datumIspitivanja-${idx}-0`)}
                    >
                      <input 
                        type="text" 
                        className="w-full h-full outline-none bg-transparent text-[13px] text-gray-800 dark:text-gray-400 px-4 py-3" 
                        value={row.datumIspitivanja[0]} 
                        onChange={e => handleSubCellChange(idx, 'datumIspitivanja', 0, e.target.value)}
                        ref={(el) => { inputRefs.current[`datumIspitivanja-${idx}-0`] = el; }}
                        data-input-key={`datumIspitivanja-${idx}-0`}
                      />
                    </td>
                    <td 
                      className="border border-gray-200 dark:border-white/[0.1] p-0 cursor-text"
                      onClick={() => handleCellClick(`datumSledecegIspitivanja-${idx}-0`)}
                    >
                      <input 
                        type="text" 
                        className="w-full h-full outline-none bg-transparent text-[13px] text-gray-800 dark:text-gray-400 px-4 py-3" 
                        value={row.datumSledecegIspitivanja[0]} 
                        onChange={e => handleSubCellChange(idx, 'datumSledecegIspitivanja', 0, e.target.value)}
                        ref={(el) => { inputRefs.current[`datumSledecegIspitivanja-${idx}-0`] = el; }}
                        data-input-key={`datumSledecegIspitivanja-${idx}-0`}
                      />
                    </td>
                    <td 
                      className="border border-gray-200 dark:border-white/[0.1] p-0 cursor-text"
                      onClick={() => handleCellClick(`napomena-${idx}-0`)}
                    >
                      <input 
                        type="text" 
                        className="w-full h-full outline-none bg-transparent text-[13px] text-gray-800 dark:text-gray-400 px-4 py-3" 
                        value={row.napomena[0]} 
                        onChange={e => handleSubCellChange(idx, 'napomena', 0, e.target.value)}
                        ref={(el) => { inputRefs.current[`napomena-${idx}-0`] = el; }}
                        data-input-key={`napomena-${idx}-0`}
                      />
                    </td>
                  </tr>
                  {[1,2,3].map(subIdx => (
                    <tr key={subIdx}>
                      <td 
                        className="border border-gray-200 dark:border-white/[0.1] p-0 cursor-text"
                        onClick={() => handleCellClick(`brojStrucnogNalaza-${idx}-${subIdx}`)}
                      >
                        <input 
                          type="text" 
                          className="w-full h-full outline-none bg-transparent text-[13px] text-gray-800 dark:text-gray-400 px-4 py-3" 
                          value={row.brojStrucnogNalaza[subIdx]} 
                          onChange={e => handleSubCellChange(idx, 'brojStrucnogNalaza', subIdx, e.target.value)}
                          ref={(el) => { inputRefs.current[`brojStrucnogNalaza-${idx}-${subIdx}`] = el; }}
                          data-input-key={`brojStrucnogNalaza-${idx}-${subIdx}`}
                        />
                      </td>
                      <td 
                        className="border border-gray-200 dark:border-white/[0.1] p-0 cursor-text"
                        onClick={() => handleCellClick(`datumIspitivanja-${idx}-${subIdx}`)}
                      >
                        <input 
                          type="text" 
                          className="w-full h-full outline-none bg-transparent text-[13px] text-gray-800 dark:text-gray-400 px-4 py-3" 
                          value={row.datumIspitivanja[subIdx]} 
                          onChange={e => handleSubCellChange(idx, 'datumIspitivanja', subIdx, e.target.value)}
                          ref={(el) => { inputRefs.current[`datumIspitivanja-${idx}-${subIdx}`] = el; }}
                          data-input-key={`datumIspitivanja-${idx}-${subIdx}`}
                        />
                      </td>
                      <td 
                        className="border border-gray-200 dark:border-white/[0.1] p-0 cursor-text"
                        onClick={() => handleCellClick(`datumSledecegIspitivanja-${idx}-${subIdx}`)}
                      >
                        <input 
                          type="text" 
                          className="w-full h-full outline-none bg-transparent text-[13px] text-gray-800 dark:text-gray-400 px-4 py-3" 
                          value={row.datumSledecegIspitivanja[subIdx]} 
                          onChange={e => handleSubCellChange(idx, 'datumSledecegIspitivanja', subIdx, e.target.value)}
                          ref={(el) => { inputRefs.current[`datumSledecegIspitivanja-${idx}-${subIdx}`] = el; }}
                          data-input-key={`datumSledecegIspitivanja-${idx}-${subIdx}`}
                        />
                      </td>
                      <td 
                        className="border border-gray-200 dark:border-white/[0.1] p-0 cursor-text"
                        onClick={() => handleCellClick(`napomena-${idx}-${subIdx}`)}
                      >
                        <input 
                          type="text" 
                          className="w-full h-full outline-none bg-transparent text-[13px] text-gray-800 dark:text-gray-400 px-4 py-3" 
                          value={row.napomena[subIdx]} 
                          onChange={e => handleSubCellChange(idx, 'napomena', subIdx, e.target.value)}
                          ref={(el) => { inputRefs.current[`napomena-${idx}-${subIdx}`] = el; }}
                          data-input-key={`napomena-${idx}-${subIdx}`}
                        />
                      </td>
                    </tr>
                  ))}
        ...
        >
          <div className="text-center">
            ...
            <p className="text-sm leading-6 text-gray-500 dark:text-gray-400">
              Smanjivanjem broja redova biće uklonjen tekst u poljima koja više neće biti vidljiva. Da li ste sigurni da želite da nastavite?
            </p>
...
export default EvidencijaIspitivanjaSredine; 